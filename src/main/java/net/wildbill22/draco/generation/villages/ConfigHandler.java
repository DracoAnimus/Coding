		...
		try {
			if (file.createNewFile()) {
				try {
					...
					LogHelper.error("ConfigHandler: " + e.getMessage());
					e.printStackTrace();
				}
			}
			...
			LogHelper.error("ConfigHandler: " + String.format("[%s] Can't load or create its config in %s.", REFERENCE.MODID, file.getAbsolutePath()));
		}
	}
